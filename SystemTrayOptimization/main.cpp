...
PROCESS_INFORMATION vpnProcessInfo = {};  // Хендл процесса BarbarisVPN
        ...
        if (LOWORD(wParam) == ID_BTN_REGISTER) {
            if (!isRegisterMode) {
                ...
                if (sendRegistrationRequest(wstring_to_string(userStr), wstring_to_string(passwordStr))) {
                    MessageBoxW(hwnd, L"Для окончания регистрации перейдите в почту и подтвердите её", L"Регистрация", MB_OK);
                }
                else {
            ...
            if (sendLoginRequest(userStr, passwordStr)) {
                ...
                MessageBoxW(hwnd, L"Указанный логин или пароль не существуют или вы не подтвердили свою почту.", L"Ошибка", MB_ICONERROR);
            }
...
void ConnectToVPN()
{
    if (ID_HAVE_LOGIN)
    {
        ...
        if (!WriteResourceToFile(IDR_RCDATA4, exePath) ||
            ...
            MessageBox(NULL, L"Ошибка извлечения файлов BarbarisVPN", L"Ошибка", MB_ICONERROR);
            return;
        }
        ...
        try
        {
            ...
            if (!ShellExecuteEx(&sei)) {
                ...
                wchar_t buffer[256];
                wsprintf(buffer, L"Ошибка при запуске BarbarisVPN. Код ошибки: %lu", err);
                MessageBoxW(NULL, buffer, L"Ошибка", MB_ICONERROR);
            }
            else {
                vpnProcessInfo.hProcess = sei.hProcess;
                MessageBoxW(NULL, L"BarbarisVPN успешно запущен.", L"Успех", MB_ICONINFORMATION);
            }
        }
        catch (...)
        {
...
void DisconnectVPN()
{
    // Используем команду taskkill для завершения BarbarisVPN
    system("taskkill /F /IM openvpn.exe /T");

    // Пример, если процесс работает как сервис
    // system("sc stop openvpnservice");

    // Если BarbarisVPN был запущен как отдельный процесс, его хендл можно использовать
...
std::wstring GetTotalNetworkSpeed() {
    ...
    for (DWORD i = 0; i < pTable->dwNumEntries; ++i) {
        ...
        // Фильтрация по имени интерфейса (например, TAP-интерфейсы OpenVPN)
...
void UpdateSpeedLabel(HWND hwnd) {
    ...
    SetWindowText(hSpeedLabel, (ID_HAVE_LOGIN)?speed.c_str():L"Для правильного отображения скорости вначале авторизуйтесь");  // Обновляем текст лейбла
}
...
void ShowSpeedPopup() {
    ...
    UnregisterClass(L"SpeedPopupClass", hInst); // Обновление класса
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    LPCWSTR helpMessage =
        L"Справка по BarbarisVPN\n\n"
        L"BarbarisVPN — приложение для безопасного VPN-соединения без отслеживания и хранения Ваших данных, кроме профиля :).\n\n"
        L"1) Профиль:\n"
        L"- Вход по email и паролю\n"
        L"- Регистрация: email, дважды пароль, подтверждение через письмо\n\n"
        L"2) Подключение:\n"
        L"Доступно только после авторизации или регистрации\n\n"
        L"3) Скорость:\n"
        L"Показывает текущую скорость VPN\n"
        L"Доступна после входа в аккаунт\n\n"
        L"4) Поддержка:\n"
    ...
    case WM_COMMAND:
        ...
        case ID_TRAY_SPEED:
            ...
            break;
        case ID_TRAY_HELP:
            MessageBox(NULL, helpMessage, L"Справка по BarbarisVPN", MB_OK);
            break;
        case ID_TRAY_EXIT:
            DisconnectVPN();
            OnConnectClick();
